...
static std::vector<std::string> consume_gemini_stream_objects(async_conversation_t& async_conversation,
    ...
    if (cursor > 0) {
        ...
        TS_LOG(ERROR) << "Malformed Gemini stream chunk: " << async_conversation.stream_remainder;
...
void GeminiConversationModel::async_res_write_callback(std::string& response, const std::shared_ptr<http_req>& req, const std::shared_ptr<http_res>& res) {
    ...
    try {
        ...
        for (const auto& object : objects) {
            ...
            if (json_res["candidates"][0].count("content") == 0 ||
                json_res["candidates"][0]["content"].count("parts") == 0) {
                continue;
            }
    ...
    try {
        ...
        for(const auto& payload : payloads) {
            ...
            try {
                ...
                TS_LOG(ERROR) << "Azure callback: Failed to parse JSON: " << payload << " Error: " << e.what();
                continue;
            }
        }
