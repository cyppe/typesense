...
Option<bool> HttpServer::handle_gzip(const std::shared_ptr<http_req>& request) {
    ...
    request->zs.next_in = reinterpret_cast<Bytef*>(request->body.data());
    ...
    do {
        ...
        if (ret != Z_STREAM_END && ret != Z_OK && ret != Z_BUF_ERROR) {
            std::string error_msg = request->zs.msg ? request->zs.msg : "inflate failed";
...
bool HttpServer::is_write_request(ReplicationService* replication_state,
    ...
    if (replication_state != nullptr) {
        bool known_route = false;
        const bool should_replicate = replication_state->should_replicate_write(route_hash, &known_route);
        if (known_route) {
