...
TEST_F(CoreAPIUtilsTest, RemoveIfFoundManyBasicBehavior) {
    ...
    ASSERT_EQ(size_t{2}, remove_op.get());
    ASSERT_EQ(size_t{2}, removed_docs.size());
    ASSERT_EQ(size_t{8}, coll1->get_num_documents());
    ...
    ASSERT_EQ(uint32_t{404}, get_1_op.code());
    ASSERT_EQ(uint32_t{404}, get_2_op.code());
...
TEST_F(CoreAPIUtilsTest, RemoveIfFoundManyWithCascadeReference) {
    ...
    ASSERT_TRUE(remove_op.ok());
    ASSERT_EQ(size_t{1}, remove_op.get());
    ...
    ASSERT_EQ(uint32_t{404}, product_get_op.code());
    ASSERT_EQ(uint32_t{404}, order_get_op.code());
...
TEST_F(CoreAPIUtilsTest, StatefulRemoveDocsUsesBoundedInternalBatch) {
    ...
    ASSERT_TRUE(remove_op.ok());
    ASSERT_EQ(uint32_t{1000}, deletion_state.num_removed);
    ASSERT_FALSE(done);

    remove_op = stateful_remove_docs(&deletion_state, 1000000000, done);
    ASSERT_TRUE(remove_op.ok());
    ASSERT_EQ(uint32_t{1205}, deletion_state.num_removed);
    ASSERT_TRUE(done);
    ASSERT_EQ(size_t{0}, coll1->get_num_documents());
