...
| 19 | NuRaft replacement feasibility sprint | P1.7d | **in progress** | Investigation says NuRaft is the only serious in-process replacement candidate, but it is a real subsystem rewrite, not a dependency bump. Stories A and B are complete, Story C has an explicit prototype implementation outline, and the first runtime seam has already landed (`ReplicationService` abstraction implemented by the current `braft` path). Next step is the isolated prototype target shell. |
