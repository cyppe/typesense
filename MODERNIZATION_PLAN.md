...
- **Decision checkpoint is now concrete enough to guide scope:** the forked `braft` path is fixed for the unhealthy-peer timed-snapshot deadlock class and remains the safer production runtime today. NuRaft is still the better recovery candidate and still worth continuing. After the single-node runtime reuse work, the direct local-apply fast path, reusable replay-progress persistence, the read-mostly document cache cleanup, repeat-support in the contention harness, and a bounded live Typesense-state mirror for valid-schema CRUD writes, the bounded API replay lane improved again to about `2.43s` collections / `2.48s` documents versus `56-58s` for `braft`, mixed contention writes reached parity in the median lane, and pure reads now slightly beat `braft` in isolation. The remaining steady-state blocker is narrower than before: sustained writes and mixed read/write interference still lag materially on the live NuRaft runtime. From here, either keep closing that write-path/runtime-integration gap with explicit follow-up tasks or stop the migration; do not pretend the decision is still blocked on missing benchmark lanes.
...
- [ ] Reduce sustained write-path interference on the live runtime by profiling local append/apply, durability, and live-state mirror costs under `--writer-threads=1 --reader-threads=0` before doing more broad tuning.
- [ ] Keep the mixed lane attributable: use the pure-read (`--writer-threads=0`) and pure-write (`--reader-threads=0`) contention variants alongside the mixed profile so regressions are tied to reads, writes, or interference instead of guessed.
