...
- [x] Replace patch-only forks with released upstream versions where possible. *(Done Mar 17, 2026: removed the stale `typesense/hnswlib` fork by pinning the same upstream `nmslib/hnswlib` parent commit directly, then repointed `clip_tokenizer_cpp` from the Typesense mirror to the identical upstream `ozanarmagan/clip_tokenizer_cpp` commit; `kakasi` remains the only documented exception in `bazel/PATCH_DEBT.md`.)*
...
| 34 | ~~Replace patch-only forks with released upstream versions where possible~~ | P1.7 Patch Debt | **done** | Completed March 17, 2026. Removed the stale `typesense/hnswlib` fork by pinning the same upstream `nmslib/hnswlib` parent commit directly while the migration was still in progress, then switched `clip_tokenizer_cpp` from the Typesense mirror to the identical upstream `ozanarmagan/clip_tokenizer_cpp` commit. Item 36 later removed the supported `hnswlib` runtime path entirely; `kakasi` remains the only documented exception in `bazel/PATCH_DEBT.md`. |
...
Completed March 17, 2026. The stale `hnsw` fork is gone, `clip_tokenizer_cpp` now points at its real upstream source repo, and `kakasi` remains the only explicit fork-backed exception.
...
- `typesense/kakasi` is not yet a drop-in upstream swap. GitHub compare against parent `loretoparisi/kakasi` still shows the fork is `ahead_by=5`, `behind_by=0`, and those five fork-only commits are material on this branch: `0933d42f` adds `data/japanese_data.{h,cpp}` that `bazel/kakasi.BUILD` links today, `a628ffd1` fixes UTF-8 library reads by using `get1byte()` instead of `getchar()`, `f82266f1` / `77f2d1ce` change bad-unicode handling from early termination to skip-and-continue, and `src/kakasi.c` now zero-initializes its `Character` buffers. Parent repo still has no GitHub release history recorded here, so keep the fork for now.
...
- **Active now:** no P1 item is currently marked active. Item **34**'s remaining fork-backed audit is now closed; the only dependency follow-up left here is conditional future work on `kakasi` if upstream absorbs the fork-only commits or publishes a usable new release/source.
...
- **Recently finished:** item **34** (`Replace patch-only forks with released upstream versions where possible`) — the stale `typesense/hnswlib` fork is gone, and `clip_tokenizer_cpp` now points at the identical upstream `ozanarmagan/clip_tokenizer_cpp` commit instead of a Typesense mirror. Item **36** later removed the supported `hnswlib` runtime path entirely. `kakasi` remains the only explicit fork-backed exception in `bazel/PATCH_DEBT.md` rather than silent future work.
...
- **Later:** revisit the remaining documented fork exception (`kakasi`) only if those 5 fork-only commits land upstream or a fresh upstream release/source materially changes the drop-in story. Monitor future sanitizer warning growth when GCC/libstdc++ or Abseil changes again, but keep any suppression file-scoped and dependency-scoped rather than broad. Mine `TODO.md` only when an item clearly aligns with the modernization queue above.
