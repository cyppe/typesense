...
| 20 | Sanitizer lane stabilization + ORT extensions boundary audit | Known Issues | **in progress** | Local-first stabilization work: normal `//:typesense-test` passes, full local ASAN is green (`1449` tests), and the focused TSAN subset is green after skipping non-actionable Whisper voice-query inference under TSAN. Root cause for the prior suite-only `_rand(seed)` failure was a missing random-sort sentinel in `populate_sort_mapping()`, which left `field_values` undefined and could route scoring into the wrong branch under ASAN. The latest hosted-only sanitizer reds were both latency-budget mismatches, not new sanitizer reports: `CollectionSpecificMoreTest.SearchCutoffTest` needed a slightly larger TSAN-only cutoff to exercise `search_cutoff` instead of returning a hard 408, and `CollectionVectorTest.TestVoiceQuery` needed a larger ASAN search deadline because the test validates voice-query integration, not timeout behavior. Remaining work is a manual GitHub sanitizer rerun on the validated local change set and the longer-term ORT Extensions registration cleanup while keeping the promoted binary self-contained. |

### Backlog map (active / later / archival)

Use this to decide what to pick next without scanning multiple files.

- **Active now (execution lane):** item **20** (`Sanitizer lane stabilization + ORT extensions boundary audit`) — local ASAN + focused TSAN are green; next step is a manual GitHub sanitizer rerun on the validated local change set, then keep the ORT Extensions boundary cleanup as follow-up architecture work.
