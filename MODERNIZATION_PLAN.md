...
- **API replay remains a distinct gap:** the prototype still does not expose the normal HTTP server/runtime surface, so the focused API-phase replay item is not yet just "run the existing harness against a new binary". Finish the current prototype-level verification first, then decide whether to add a thin HTTP-facing prototype adapter or keep API replay out of scope for the feasibility sprint.
...
- **Canonical recovery comparison harness landed:** `scripts/benchmark_vs_upstream.sh --profile raft-recovery` now stages the current fork's `typesense-server`, runs a real 3-node `braft` follower-outage/rejoin scenario, runs the NuRaft `snapshot-policy-compare` benchmark with matching write/outage counts, and emits one JSON summary under `~/.cache/typesense/benchmark/raft-recovery-summary.json`. Story E no longer depends on ad hoc scripts for the first real side-by-side recovery measurement.
- **First repeatable recovery comparison now exists:** with `--docs=200 --post-snapshot-docs=50 --snapshot-rounds=3 --repeats=2`, NuRaft's leader-only policy recovered in roughly `0.52ms` after latest-snapshot install while `require-healthy-peers` added about `150` replayed entries and roughly `10.94ms` of extra recovery time inside the prototype lane. The live `braft` runtime recovered in roughly `3.26s`, preserved snapshot freshness to within `2` entries during the outage, and still needed roughly `152-153` replayed entries on follower restart in these runs.
- **Decision checkpoint is clearer, but still not the final go/no-go:** the forked `braft` path is now fixed for the unhealthy-peer timed-snapshot deadlock class and should stay in production for now, while NuRaft's recovery behavior is good enough to justify continued evaluation. The remaining Story E gap is steady-state CPU/memory and broader contention/runtime parity, not whether the recovery lane is worth pursuing at all.
