...
- **A canonical runtime contention lane now exists too:** `scripts/benchmark_vs_upstream.sh --profile raft-runtime-contention` compares one writer plus bounded document readers against the live `braft` server and the live NuRaft runtime on preloaded document ids. The first canonical run exposed the self-lock bug and a severe per-request reconstruction cost. After keeping the single-node request journal, state machine, materialized-state sink, and replay-progress persistence alive across requests, then adding a direct single-node append/apply fast path and persistent segment-log appends, the same lane now repeats around `3,388-3,448` writes / `9,061-9,102` reads for NuRaft versus `5,179-5,198` writes / `31,197-31,373` reads for `braft` in clean `5s` runs. A later single-node document-cache pass kept API replay strong, but fresh contention reruns on this host were too noisy to replace that band, so treat the Run 19 band as the current steady-state checkpoint until a quieter rerun says otherwise. That is still a steady-state loss, especially on reads, but it is now in "optimize further or stop" territory rather than "obvious no-go".
...
- **Decision checkpoint is now concrete enough to guide scope:** the forked `braft` path is fixed for the unhealthy-peer timed-snapshot deadlock class and remains the safer production runtime today. NuRaft is still the better recovery candidate and still worth continuing. After the single-node runtime reuse work, the direct local-apply fast path, reusable replay-progress persistence, and a small single-node document cache, the steady-state contention gap is still a real but tractable optimization problem rather than a clear no-go signal, while the bounded API replay lane improved again to about `2.43s` collections / `2.48s` documents versus `56-58s` for `braft`. From here, either keep closing the read-path/runtime-integration gap with explicit follow-up tasks or stop the migration; do not pretend the decision is still blocked on missing benchmark lanes.
