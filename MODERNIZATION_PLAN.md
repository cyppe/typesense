...
- [ ] Expand the bounded API replay from health/status, collection/document write/read, and snapshot flows into broader product-response parity before attempting the full suite.
...
- **Thin HTTP runtime lane landed:** `//:typesense-server-nuraft-runtime` now exposes a bounded NuRaft-backed HTTP surface on top of the existing `HttpServer`: `GET /health`, `GET /status`, collection/document fetch, collection/document writes, and `POST /operations/snapshot`, with subprocess-backed coverage in `//:nuraft-http-runtime-test` for single-node restart/snapshot and snapshot-install-into-fresh-node flows. This moves the focused API replay gap from "no HTTP/runtime surface exists" to "expand the bounded surface and run more of the real harness against it".
- **Focused API replay now works through the real wrapper:** `scripts/run_api_tests.sh --server-binary ./bazel-bin/typesense-server-nuraft-runtime -- --no-secrets tests/nuraft_runtime_smoke.test.ts` now passes the `single-fresh`, `single-restarted`, and `single-snapshot` phases end to end, and the API runner now supports an explicit `--single-node-only` flag when you want to reuse only the single-node phases of an existing file while keeping the broader multi-node path available.
- **Bounded multi-node API replay now works too:** the same `scripts/run_api_tests.sh` wrapper now passes full `tests/health.test.ts` and the new `tests/nuraft_runtime_cluster.test.ts` against `//:typesense-server-nuraft-runtime`, exercising `multi-fresh`, `multi-restarted`, and `multi-snapshot` phases with follower-originated writes, leader forwarding, cross-node reads, and cluster restart persistence through the static-cluster runtime options.
- **Existing collection/document API files now pass unchanged on the bounded runtime lane:** `tests/collections.test.ts` and `tests/documents.test.ts` now pass end to end against `//:typesense-server-nuraft-runtime` through the real `scripts/run_api_tests.sh` wrapper. This required flattening the bounded collection/document response shape toward the product surface, adding PATCH/delete document routes plus merge semantics, and adding a minimal materialized-state `/documents/search` path for the focused query cases covered by the existing file.
...
- **HTTP async follow-up is now explicit:** the thin runtime currently keeps NuRaft write/snapshot HTTP responses on a simplified synchronous path after mixed `GET` then write flows exposed a crash in the first async handler attempt. If NuRaft stays on the table past the feasibility sprint, revisit async response/import/streaming parity before treating `typesense-server-nuraft-runtime` as a production-like integration surface.
- **Benchmarking constraint is now explicit:** the repo's existing `scripts/benchmark_vs_upstream.sh` and `benchmark/` CLI are HTTP-server benchmarks, so they are useful for the current `braft` path but do not directly compare against the isolated NuRaft prototype binary. Story E will need either a small prototype-specific microbenchmark harness or a later HTTP-facing NuRaft adapter; do not pretend the current benchmark wrapper answers the NuRaft go/no-go question yet.
...
- [x] Benchmark the prototype against the current `braft` path for commit latency, write throughput, follower catch-up speed, snapshot creation/install time, and steady-state CPU/memory cost.
...
- **A canonical focused runtime-vs-runtime timing lane now exists:** `scripts/benchmark_vs_upstream.sh --profile raft-api-replay` measures the same real API wrapper files against `//:typesense-server` and `//:typesense-server-nuraft-runtime`, currently `tests/collections.test.ts` and `tests/documents.test.ts`. The first run produced `~3.53s` vs `~59.26s` for collections and `~2.58s` vs `~57.20s` for documents in favor of the bounded NuRaft runtime. This is useful apples-to-apples runtime-path evidence on implemented surfaces, but it is still a focused API replay timing lane, not a replacement for the broader benchmark dashboard profiles.
- **A canonical runtime contention lane now exists too:** `scripts/benchmark_vs_upstream.sh --profile raft-runtime-contention` compares configurable document writers and readers against the live `braft` server and the live NuRaft runtime on preloaded document ids. The first canonical run exposed the self-lock bug and a severe per-request reconstruction cost. After keeping the single-node request journal, state machine, materialized-state sink, and replay-progress persistence alive across requests, then adding a direct single-node append/apply fast path and persistent segment-log appends, the lane reached a clean Run 19 band of `3,388-3,448` writes / `9,061-9,102` reads for NuRaft versus `5,179-5,198` writes / `31,197-31,373` reads for `braft`. A later read-mostly cache cleanup plus repeat-support in the harness moved the mixed median of three repeats to rough write parity (`3,415` NuRaft vs `3,260` `braft`) while reads remained far behind (`9,042` NuRaft vs `42,517` `braft`). The isolation runs narrowed that result: pure reads with `--writer-threads=0 --reader-threads=2` were already slightly ahead on NuRaft (`57,204` vs `53,928`). The decisive change came when the single-node runtime stopped fsyncing replay-progress metadata on every write and instead derived local progress from the materialized sink: pure writes flipped from a `0.47x` loss to a `1.48x` win (`6,606` vs `4,458`), and a fixed-rate mixed lane (`--writer-interval-ms=2`) now shows near-equal writes (`3,246` vs `3,208`) with about `91%` of `braft` read throughput (`85,642` vs `94,042`) while still using much less CPU and RSS. That means the bounded steady-state contention story is now good enough for a provisional `go`: the remaining work is broader parity and hardening, not proving basic viability.
...
- **Canonical recovery comparison harness landed:** `scripts/benchmark_vs_upstream.sh --profile raft-recovery` now stages the current fork's `typesense-server`, runs a real 3-node `braft` follower-outage/rejoin scenario plus a late-third-node join scenario, runs the NuRaft `snapshot-policy-compare` and `delayed-join` benchmarks with matching write counts, and emits one JSON summary under `~/.cache/typesense/benchmark/raft-recovery-summary.json`. Story E no longer depends on ad hoc scripts for the first real side-by-side recovery measurement.
...
- **Decision checkpoint is now concrete enough to guide scope:** the forked `braft` path is fixed for the unhealthy-peer timed-snapshot deadlock class and remains the safer production runtime today, but the benchmark question is no longer undecided. NuRaft is now the preferred replacement direction for continued work: recovery is materially better, the bounded API replay lane is materially faster, pure reads slightly beat `braft`, pure writes now beat `braft` after the local replay-progress change, and a fixed-rate mixed lane keeps about `91%` of `braft` read throughput at equal write pressure while using much less CPU and RSS. That is enough for a provisional `go` on NuRaft as the migration target. It is **not** enough to delete `braft` immediately: broader API parity, async/import/streaming hardening, and an explicit cutover plan still need to land first.

**If NuRaft remains active after this checkpoint, the next implementation tasks should be explicit:**

- [ ] Extend the new single-node runtime reuse work carefully: keep long-lived journal/state/sink handles where they help, but do not break the static-cluster emulation path that still touches follower dirs across processes.
- [ ] Keep the mixed lane attributable: use the pure-read (`--writer-threads=0`), pure-write (`--reader-threads=0`), and fixed-rate mixed (`--writer-interval-ms=2`) contention variants together so regressions are tied to reads, writes, or interference instead of guessed.
...
- [ ] Re-run `raft-api-replay`, `raft-runtime-contention`, and `raft-recovery` together after each runtime optimization pass; do not optimize against a single lane.
...
| 19 | NuRaft replacement feasibility sprint | P1.7d | **in progress** | Investigation says NuRaft is the only serious in-process replacement candidate, but it is a real subsystem rewrite, not a dependency bump. Stories A and B are complete, and Story C now has the isolated single-node path, static three-node catch-up path, external snapshot/export parity, and a bounded membership/IP-drift policy without pulling NuRaft into `//:typesense-server`. Story D now includes a thin recovery harness plus coverage for leadership switches, lag visibility, snapshot-installed follower catch-up, restart-style delta replay, divergence rejection, chunked-import forwarding across leader changes, expanded bootstrap peer refresh, and the unhealthy-peer timed-snapshot deadlock class. Story E now has the prototype benchmark target plus canonical `raft-recovery`, `raft-api-replay`, and `raft-runtime-contention` profiles in `scripts/benchmark_vs_upstream.sh`. Current checkpoint: NuRaft is still winning recovery behavior and the bounded API replay lane (latest focused replay: `~2.43s` collections / `~2.48s` documents versus `56-58s` for `braft`), pure reads on the bounded runtime lane are now slightly ahead (`57,204` vs `53,928`), but sustained pure writes (`2,245` vs `4,736`) and mixed read/write contention still lag enough that `braft` remains the safer production path today. |
| 19 | NuRaft replacement feasibility sprint | P1.7d | **in progress** | Investigation says NuRaft is the only serious in-process replacement candidate, but it is a real subsystem rewrite, not a dependency bump. Stories A and B are complete, and Story C now has the isolated single-node path, static three-node catch-up path, external snapshot/export parity, and a bounded membership/IP-drift policy without pulling NuRaft into `//:typesense-server`. Story D now includes a thin recovery harness plus coverage for leadership switches, lag visibility, snapshot-installed follower catch-up, restart-style delta replay, divergence rejection, chunked-import forwarding across leader changes, expanded bootstrap peer refresh, and the unhealthy-peer timed-snapshot deadlock class. Story E now has the prototype benchmark target plus canonical `raft-recovery`, `raft-api-replay`, and `raft-runtime-contention` profiles in `scripts/benchmark_vs_upstream.sh`. Current checkpoint: NuRaft is the provisional benchmark winner and the preferred migration direction. It is still winning recovery behavior and the bounded API replay lane (`~2.43s` collections / `~2.48s` documents versus `56-58s` for `braft`), pure reads are slightly ahead (`57,204` vs `53,928`), pure writes now beat `braft` after the local replay-progress change (`6,606` vs `4,458`), and a fixed-rate mixed lane keeps roughly `91%` of `braft` read throughput (`85,642` vs `94,042`) at equal write pressure with much lower CPU/RSS. Keep `braft` as the production fallback until broader parity and cutover work are done, but active engineering should now assume NuRaft is the intended replacement path. |

### Backlog map (active / later / archival)

Use this to decide what to pick next without scanning multiple files.

- **Active now (execution lane):** item **18** (`Dependency refresh audit`) remains the primary modernization lane. Item **19** (`NuRaft replacement feasibility sprint`) is also in progress by explicit user request; Stories A/B are done, Story C now has the single-node, static-cluster, snapshot/export, and bounded membership/IP-drift milestones checked off, Story D has focused recovery/leadership verification in place, and Story E now has canonical recovery, API-replay, and runtime-contention comparison lanes. The benchmark question is now sufficiently answered to keep moving: NuRaft is the preferred replacement direction, while `braft` stays in-tree as the production fallback until broader parity and migration-hardening tasks are complete. The next NuRaft work should bias toward broader runtime/API parity plus async/import/streaming hardening instead of more open-ended feasibility probing.
...
35. **Timed snapshots in the current `braft` path should not be blocked by unhealthy followers.** Snapshot freshness is part of follower recovery, not a reward for already being healthy. Keep a direct regression test on the production `raft_server` policy so the deadlock class stays fixed in this fork while NuRaft feasibility work continues.
36. **`braft` timed snapshots are currently gated by the 60-second maintenance loop, not just `--snapshot-interval-seconds`.** Lowering `snapshot-interval-seconds` alone does not make snapshots fire every second, because `typesense_server_utils.cpp` only calls `ReplicationState::do_snapshot()` on `raft_counter % 60 == 0`. Any benchmark or recovery experiment that expects timed snapshots sooner must either wait long enough to cross that loop or change the scheduler.
37. **A fresh `braft` snapshot does not guarantee follower snapshot install on rejoin.** In the current runtime experiments, the leader advanced timed snapshots to within `2` entries of the final committed index, but the recovering follower still replayed roughly the entire outage window from retained logs instead of obviously taking an `InstallSnapshot` path. Benchmark both snapshot freshness and replay gap instead of assuming one implies the other.
