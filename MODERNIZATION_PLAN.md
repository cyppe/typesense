...
- **Target shape that survived the cutover:** keep prototype-only code separate from `//:typesense-server`. The surviving split is `nuraft_prototype_lib` + `typesense-server-nuraft-prototype` for isolated prototype work and `nuraft_runtime_lib` + `//:typesense-server` for the real server path.
- **Recommended on-disk isolation:** keep all prototype state under `state/nuraft-prototype/{meta,log,snapshot}`. The old `braft` state-sharing concern is now historical, but the prototype/runtime split still matters for test isolation and safe iteration.
- **Milestone order:**
...
- **Benchmarking constraint is now explicit:** the repo's existing `scripts/benchmark_vs_upstream.sh` and `benchmark/` CLI are HTTP-server benchmarks, while `//:nuraft-prototype-benchmark` remains the prototype-only measurement lane. The old dual-runtime Raft comparison profiles are archival only on this branch.
...
- [ ] Fix the `single-restarted` mixed-state startup crash from the full `api_tests --no-secrets` replay, then keep that full wrapper as the main runtime regression lane instead of relying only on per-suite targeted replays.
- [ ] Extend multi-node parity beyond CRUD/import: replicate and serve `synonym_sets`, `curation_sets`, and `analytics` correctly across the static-cluster runtime path, then use the same pattern for the remaining registered product-write families that still rely on direct product handlers.
...
| 19 | NuRaft replacement cutover and hardening | P1.7d | **in progress** | The decision phase is over on this branch: `braft`/`brpc`/`glog` are removed, `//:typesense-server` is now the NuRaft-backed runtime, and the old side-by-side comparison lanes are archival only. Stories A-E supplied the evidence for the cutover; the remaining work is now broader product-route parity beyond CRUD/synonyms/analytics, plus async/import/streaming hardening and continued runtime tuning. |

### Backlog map (active / later / archival)

Use this to decide what to pick next without scanning multiple files.

- **Active now (execution lane):** item **18** (`Dependency refresh audit`) remains the primary modernization lane. Item **19** is no longer a feasibility question on this branch; it is now a NuRaft-only hardening lane. The benchmark question was answered strongly enough to remove the old stack here, so the next NuRaft work should bias toward broader runtime/API parity plus async/import/streaming hardening instead of more open-ended feasibility probing.
