...
- **Prototype snapshot/export groundwork landed:** `NuRaftSnapshotCoordinator` can now capture an installable snapshot of the isolated prototype meta/log/materialized-state tree, export it under `state/nuraft-prototype/` at an external path, and install it back while preserving the target node's local identity/bootstrap metadata. When the live KV sink is available, the materialized-state export now uses a RocksDB checkpoint instead of a raw directory copy. The prototype CLI exposes `--create-snapshot`, `--install-snapshot`, and `--dump-snapshot-descriptor`, covered by `//:nuraft-snapshot-coordinator-test` plus expanded controller coverage. This is snapshot groundwork, not yet full checkpoint/API-harness parity.
...
| 19 | NuRaft replacement feasibility sprint | P1.7d | **in progress** | Investigation says NuRaft is the only serious in-process replacement candidate, but it is a real subsystem rewrite, not a dependency bump. Stories A and B are complete, Story C now has both the single-node and static three-node append/commit/catch-up checkpoints done, and the prototype also has first-pass snapshot/export tooling for its isolated meta/log/materialized-state tree. Next step is to upgrade that snapshot path to closer RocksDB-checkpoint/API-harness parity, then tackle membership/IP-drift behavior, while keeping the prototype isolated from `//:typesense-server`. |

### Backlog map (active / later / archival)

Use this to decide what to pick next without scanning multiple files.

- **Active now (execution lane):** item **18** (`Dependency refresh audit`) remains the primary modernization lane. Item **19** (`NuRaft replacement feasibility sprint`) is also in progress by explicit user request; Stories A/B are done and Story C now has the single-node plus static three-node checkpoints checked off, with snapshot/export groundwork also in place, so the next NuRaft work should stay bounded to finishing snapshot parity and the remaining Story D sprint items rather than turning into an unscoped rewrite.
