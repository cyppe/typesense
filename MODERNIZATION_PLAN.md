...
- **Decision checkpoint is now concrete enough to guide scope:** the forked `braft` path is fixed for the unhealthy-peer timed-snapshot deadlock class and remains the only production-ready runtime today. NuRaft is still the better recovery candidate and still worth continuing. After the single-node runtime reuse work, the steady-state contention gap is now a real but tractable optimization problem rather than a clear no-go signal. From here, either keep closing the read-path/runtime-integration gap with explicit follow-up tasks or stop the migration; do not pretend the decision is still blocked on missing benchmark lanes.
