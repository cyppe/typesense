...
- **Prototype target shell landed:** `//:typesense-server-nuraft-prototype` now builds as an isolated compile-only shell with its own `src/main/typesense_nuraft_prototype.cpp` entrypoint and `src/nuraft/nuraft_replication_controller.cpp`, while `//:typesense-server` still builds unchanged.
...
| 19 | NuRaft replacement feasibility sprint | P1.7d | **in progress** | Investigation says NuRaft is the only serious in-process replacement candidate, but it is a real subsystem rewrite, not a dependency bump. Stories A and B are complete, the `ReplicationService` seam is landed, and `//:typesense-server-nuraft-prototype` now exists as an isolated shell target. Next step is Story C single-node boot/write/restart scaffolding behind that target. |
