...
| 37 | Release workflow promotion / draft-posture cleanup | P2 Release | **active** | The next current-tip replay (`23234876467` on `7ab4cd8b`, Mar 18, 2026) exposed a real Darwin-only Bazel 9 regression: the host `--repository_cache` lived under `GITHUB_WORKSPACE`, so Bazel's derived repo-contents cache landed inside the checkout and both macOS lanes failed before analysis. Fix the host-cache root, rerun Darwin validation, then finish the stale-wording cleanup. |
| 38 | Indexing hot-path string-copy audit | P2 Perf | **active** | The first measured candidate removes a deep JSON copy when handing parsed docs into `index_record`; `quick/core` import improved `27.716s -> 26.392s` (`-4.78%`) on Mar 18, 2026, but the same run's search p95 table was mixed and a fresh-workdir rerun hit a benchmark-harness Influx path error, so closeout still needs one cleaner repeat. |
...
- The supported release contract is now one manual `release-binaries.yml` workflow plus the repo-owned local replay wrappers, and the latest completed full hosted proof on this branch is still green (`23129745485` on March 16, 2026 across `linux-amd64`, `linux-arm64`, `linux-arm64-lg-page16`, `darwin-arm64`, and `darwin-amd64`).
- The first current-tip cache-priming `release-binaries` replay on this line (`23234876467` on SHA `7ab4cd8b`, March 18, 2026) found a real Darwin-only regression under Bazel 9 before the release-posture cleanup could be closed.
...
- The remaining item-37 work is now split: first repair the new macOS host-cache regression, then remove the stale plan-only "draft/promotion follow-up" wording once a fresh Darwin/current-tip replay is green.
...
- [x] Audit workflow names, comments, README/runbook wording, and helper-script help text for stale "draft", "proof-only", or pre-GPU artifact assumptions.
- [x] Verify whether any real blocker remains to calling `release-binaries.yml` the supported manual release lane on this branch.
...
- Source inspection of `.github/workflows/release-binaries.yml`, `TESTING_RUNBOOK.md`, `README.md`, `scripts/release_linux_artifacts.sh`, and `scripts/publish_release.sh` found that the supported-manual-release posture is already reflected in the workflow name, wrapper help text, and runbook/README wording. The stale "draft" language is now concentrated in historical `MODERNIZATION_PLAN.md` notes rather than in the active workflow/docs surface.
- Updated GitHub status on March 18, 2026 is: `tests` run `23234858444` on SHA `7ab4cd8b` finished `success`; current-tip `release-binaries` run `23234876467` on the same SHA is still overall `in_progress`, but both Darwin jobs already failed (`67536421340` and `67536421335`) while `linux-arm64` and `linux-arm64-lg-page16` succeeded and `linux-amd64` kept running.
- The failing Darwin step is the host-Bazel path in `.github/workflows/release-binaries.yml`, not release packaging logic: the workflow restored `TYPESENSE_HOST_BAZEL_CACHE_DIR` under `${{ github.workspace }}` and passed `--repository_cache="${HOST_CACHE_DIR}/repository-cache"`. Bazel 9 derives `--repo_contents_cache` to `{--repository_cache}/contents` by default, so the contents cache ended up inside the checkout and Bazel aborted before analysis with "The repo contents cache ... is inside the main repo ...".
- The narrow fix is to move the host cache root outside `GITHUB_WORKSPACE` (use `${{ runner.temp }}/typesense-bazel-host`), keep `actions/cache` pointed at that absolute path, and add explicit `permissions: contents: read` per the current `actions/checkout` guidance. Validate with a fresh Darwin-scoped `release-binaries` replay before closing the stale-wording cleanup.
...
- **Active now:** items **37** and **38** are both in flight. Item **37** has completed the source audit but is waiting on the current-tip cache-priming `release-binaries` run `23233018314` before plan-only stale wording can be cleaned up honestly. Item **38** now has a first measured import win from a move-only `index_record` handoff change, but it still needs one cleaner benchmark repeat before closeout because the successful `quick/core` search p95 table was mixed and the `/tmp` rerun hit a benchmark-harness Influx path error.
...
61. **Benchmark reruns with an alternate `--work-dir` can fail in the current Dockerized k6/Influx setup.** The March 18, 2026 item-38 rerun under `/tmp/typesense-bench-move-r2` hit repeated `Couldn't write stats ... mkdir /var/lib/influxdb/data: no such file or directory` errors from k6's Influx output path. Until that harness bug is understood, prefer the default `~/.cache/typesense/benchmark` work dir for benchmark repeats and treat this failure mode as harness-only, not Typesense signal.
