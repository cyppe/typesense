...
| 37 | Release workflow promotion / draft-posture cleanup | P2 Release | **active** | Source audit says the supported-manual-release posture is already reflected in workflow/help/docs, but the current-tip cache-priming `release-binaries` run `23233018314` (Mar 18, 2026) is still `queued`/`in_progress`, so final stale-wording cleanup stays open until that run settles. |
...
- The new current-tip cache-priming `release-binaries` run (`23233018314` on SHA `86587197`, March 18, 2026) is still `queued`/`in_progress`, so this sprint needs to keep the current-tip status wording honest instead of assuming that proof is already complete.
...
- Current GitHub status on March 18, 2026 is: `tests` run `23233009256` on SHA `86587197` is `in_progress`; current-tip `release-binaries` run `23233018314` on SHA `86587197` is still overall `queued` while `linux-amd64` / `linux-arm64` are `in_progress`; older accidental `release-binaries` run `23233008625` on SHA `8610001f` is `cancelled`.
- Because the only current-tip release-lane delta is the new Bazel-cache restore/save plumbing from commit `86587197`, this sprint should keep item 37 open until run `23233018314` settles, then remove the remaining stale plan-only "draft/promotion follow-up" wording if that cache-priming replay stays green.
