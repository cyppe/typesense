...
- [ ] Expand the bounded API replay from the current single-node subset into multi-node write/read phases plus broader product-response parity before attempting the full suite.
...
- **Focused API replay now works through the real wrapper:** `scripts/run_api_tests.sh --server-binary ./bazel-bin/typesense-server-nuraft-runtime -- --no-secrets tests/nuraft_runtime_smoke.test.ts` now passes the `single-fresh`, `single-restarted`, and `single-snapshot` phases end to end, and the wrapper auto-applies `--single-node-only` for the current NuRaft runtime flavor so existing files like `tests/health.test.ts` can be replayed unchanged against the bounded single-node lane.
...
- **API replay remains a bounded gap, not a missing surface:** the thin runtime exists now, but it still covers only health/status, collection/document fetch, basic writes, and snapshot flows. Expand that surface carefully and verify async/import/streaming behavior before pointing the broader `api_tests` harness at it.
