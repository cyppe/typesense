...
- [ ] **Audit visible build warnings across normal and sanitizer builds:** Goal: zero warnings visible during any build config without hiding new regressions. Triage each warning bucket as one of: first-party fix, third-party scoped suppression, or unavoidable toolchain/startup noise. Known sources:
  ...
  - first-party `src/system_metrics.cpp`: unused and possibly-uninitialized locals under GCC 14 builds; fix in source rather than suppressing globally
...
| 22 | Release workflow promotion / current-tip full-matrix replay | Known Issues | **pending** | After item 20 lands, rerun `release-binaries` across the full matrix on the current branch tip, then do one versioned dry-run of the artifact/publish path before treating the draft workflow as release-ready. Prior full-matrix proof exists on `749f003e` (`23009183654`), but that should be refreshed on the final pre-release SHA. |

### Backlog map (active / later / archival)

Use this to decide what to pick next without scanning multiple files.

- **Active now (execution lane):** item **20** (`Sanitizer lane stabilization + ORT extensions boundary audit`) — benchmark confirmation is no longer the blocker, and the release blocker is now cleared: the full-matrix `release-binaries` rerun on March 12, 2026 (`23009183654`) validated commit `749f003e` on both Darwin lanes after forcing `GGML_BLAS=OFF`. The next active subtask is the ORT Extensions registration/teardown cleanup while preserving self-contained release artifacts and the current sanitizer guardrails.
- **Recently finished:** item **21** (`NuRaft import/runtime parity + benchmark refactor sprint`) — hosted `benchmark-testing` is green on `27bb2bff` after the final replay-model fix. Item **18** (`Dependency refresh audit`) — all actionable deps at latest, patch debt at minimum. Item **19** (`NuRaft cutover`) — 120/120 API tests. Item **9** (`Protobuf 34`) — 34.0.bcr.1.
...
- **Next after item 20:** item **22** (`Release workflow promotion / current-tip full-matrix replay`) — rerun `release-binaries` across all four lanes on the current tip, then do a versioned dry-run of the release artifact/publish path so the branch has a fresh all-arch proof immediately before release.
- **Later (planned but not started):** Compile warning audit/cleanup across normal, ASAN, and TSAN builds: fix real first-party warnings (for example `src/system_metrics.cpp`), then surgically suppress only audited third-party/toolchain noise from protobuf, abseil, and libstdc++ regex so new warning buckets still stay visible. Env-dependent test suites (blocked on infrastructure). Abseil `20260107.1` upgrade (blocked by ORT ABI mismatch). Release workflow follow-up: keep local Linux replay as the first validation step, because stale smoke-test assertions like the old `Command line usage:` grep can break packaging even when the binary itself is healthy, and consider extracting Linux tarball/package assembly into a repo-owned wrapper so CI and local replay stop depending on workflow-only shell blocks.
...
- Important caution: `.github/workflows/release-binaries.yml` is still just a draft. Cross-platform packaging validation is now complete, including the March 12, 2026 full-matrix rerun on `749f003e`; the remaining work is promotion/cleanup and adjacent ORT-boundary hardening, not basic release-lane break-fixing.
