...
1. [ ] Re-check the real DDEV lane on a fresh image built from the post-helper-rewrite branch head.
   Current state: the March 22, 2026 local `category_fanout` and `mixed_category_fitment` lanes are now green on current HEAD, including the harsher `300k` fanout / `200k` fitment / `thread_pool_size=4` mixed replay where the long `categories_se` import took `24.4s` but concurrent fitment imports still averaged `140.6ms` with `/health 0.3ms`, `/metrics.json 1.6ms`, and zero replay-side failures. The earlier Laravel timeout evidence came from an older image before the async-reference helper rewrite, so the next meaningful gate is DDEV on a freshly built image from this newer runtime.
