...
- **Decision checkpoint is clearer, but still not the final go/no-go:** the forked `braft` path is now fixed for the unhealthy-peer timed-snapshot deadlock class and should stay in production for now, while NuRaft's recovery behavior is good enough to justify continued evaluation. The late-join/add-back results strengthen that conclusion. The remaining Story E gap is broader runtime parity and contention evidence, not whether the recovery lane is worth pursuing at all.
