...
- **A canonical runtime contention lane now exists too:** `scripts/benchmark_vs_upstream.sh --profile raft-runtime-contention` compares one writer plus bounded document readers against the live `braft` server and the live NuRaft runtime on preloaded document ids. The first canonical run (`--duration 5s --docs 100 --reader-threads 2`) showed that the current NuRaft runtime no longer crashes under concurrent access after the shared-DB fix, but it still completed only about `298` writes / `438` reads versus `4,646` writes / `25,949` reads for `braft`. Treat that as current runtime-integration debt, not as a library verdict, because the NuRaft lane still reconstructs too much state per request.
...
- **Decision checkpoint is now concrete enough to guide scope:** the forked `braft` path is fixed for the unhealthy-peer timed-snapshot deadlock class and remains the only production-ready runtime today. NuRaft is still the better recovery candidate and still worth continuing, but it is not yet the replacement winner because the current runtime loses badly on bounded steady-state document read/write contention. From here, either close that runtime-integration gap with explicit follow-up tasks or stop the migration; do not pretend the decision is still blocked on missing benchmark lanes.

**If NuRaft remains active after this checkpoint, the next implementation tasks should be explicit:**

- [ ] Keep a long-lived materialized-state sink / DB handle in the runtime path instead of reconstructing sink/state-machine objects per request.
- [ ] Reduce request-time read overhead further by preferring direct keyed lookups and bounded prefix scans over whole-state reconstruction anywhere it still exists.
