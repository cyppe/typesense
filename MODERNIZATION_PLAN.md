...
- **Active now:** no P1 item is currently marked active. The recommended next follow-up is a focused audit of the remaining fork-backed exceptions (`kakasi`, `clip_tokenizer_cpp`) from item **34** / `bazel/PATCH_DEBT.md`.
...
- **Recently finished:** item **34** (`Replace patch-only forks with released upstream versions where possible`) — the stale `typesense/hnswlib` fork is gone. The repo pinned the same upstream `nmslib/hnswlib` parent commit directly during migration, and item **36** later removed the supported `hnswlib` runtime path entirely. `kakasi` and `clip_tokenizer_cpp` remain explicit exceptions in `bazel/PATCH_DEBT.md` rather than silent future work.
