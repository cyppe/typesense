...
- **Active now (execution lane):** item **20** (`Sanitizer lane stabilization + ORT extensions boundary audit`) — benchmark confirmation is no longer the blocker; remaining work is the macOS release replay after forcing `GGML_BLAS=OFF` for whisper's CPU-only build, then the follow-on ORT Extensions registration cleanup.
