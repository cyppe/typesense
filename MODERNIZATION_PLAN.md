...
- **API replay remains a bounded parity gap, not a missing runtime lane:** the thin runtime now covers single-node and static multi-node health/status, collection/document fetch, basic writes, and snapshot flows through the real `api_tests` wrapper. The remaining work is broader product-response parity plus async/import/streaming behavior, not "build an HTTP lane from scratch".
