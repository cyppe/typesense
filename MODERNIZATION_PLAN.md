...
- **Active now (no new execution lane selected):** item **22** is now closed: current-tip `release-binaries` run `23088513187` is green across all four lanes on `a2832b63`, and the versioned publish dry-run is also green after fixing `publish_release.sh` to match the actual RPM artifact names.
...
- **Next likely lane:** Compile warning audit/cleanup across normal, ASAN, and TSAN builds: fix real first-party warnings first, then surgically suppress only audited third-party/toolchain noise so new warning buckets stay visible.
