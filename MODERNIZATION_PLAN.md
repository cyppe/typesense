...
- **First direct `braft` outage baseline now exists too, but it is not apples-to-apples yet:** ad hoc multi-node trials against the real `//:typesense-server` runtime showed follower restart/catch-up around the low single-digit seconds for a small outage window, and a longer outage confirmed the fixed `braft` path now does create timed snapshots with an unhealthy follower. However, those runs also showed the current `braft` server only evaluates timed snapshots on the 60-second maintenance loop in `typesense_server_utils.cpp`, so Story E still needs a cleaner comparison harness before the go/no-go call.
