...
- [x] Replace patch-only forks with released upstream versions where possible. *(Done Mar 17, 2026: removed the stale `typesense/hnswlib` fork by pinning the same upstream `nmslib/hnswlib` parent commit directly; `kakasi` and `clip_tokenizer_cpp` remain documented exceptions in `bazel/PATCH_DEBT.md` because they are not drop-in upstream release swaps today.)*
...
| 34 | ~~Replace patch-only forks with released upstream versions where possible~~ | P1.7 Patch Debt | **done** | Completed March 17, 2026. Removed the stale `typesense/hnswlib` fork by pinning the same upstream `nmslib/hnswlib` parent commit directly. Upstream `v0.8.0` is still a no-go because it drops APIs that `src/index.cpp` uses; `kakasi` and `clip_tokenizer_cpp` remain documented exceptions in `bazel/PATCH_DEBT.md`. |
...
Completed March 17, 2026. One stale fork was removed (`hnsw`), and the remaining fork-backed deps are now documented as explicit exceptions instead of vague future work.
...
- [ ] Audit the remaining fork-backed deps (`kakasi`, `clip_tokenizer_cpp`) to decide whether they can be moved to upstream releases or must stay forked for now.

**Story A findings / implementation (Mar 17, 2026)**

- `typesense/hnswlib` was a stale fork. GitHub compare against `nmslib/hnswlib` showed `ahead_by=0`, `behind_by=54`, so the fork carried no fork-only commits. The repo now pins the same upstream parent commit `687d981753f8bafcd16421cbd2a166d0b62bc520` directly in `MODULE.bazel`, and the legacy `cmake/hnsw.cmake` helper now downloads that same upstream commit tarball instead of a Typesense fork tarball.
- Attempting the upstream `nmslib/hnswlib` `v0.8.0` release was a concrete no-go, not guesswork. The canonical Dockerized build failed because `src/index.cpp` still relies on `searchKnnCloserFirst(..., ef, filter)` and `repair_zero_indegree()`, which are not present in `v0.8.0`.
- `typesense/kakasi` is not yet a drop-in upstream swap. GitHub compare against parent `loretoparisi/kakasi` shows the fork is `ahead_by=5`, `behind_by=0`, and the parent repo has no GitHub release history recorded here, so those five fork-only commits need an explicit audit before we can replace it.
- `typesense/clip_tokenizer_cpp` is not a GitHub fork and no upstream release path is recorded here yet, so it stays as-is until that upstream project and release contract are identified.
- Canonical proof for the `hnsw` fork removal is green:
  - `scripts/bazel_in_docker.sh build //:typesense-server`
...
- The entrypoint surface is now explicit by folder:
  ...
  - `test/scripts/` owns focused test-scoped helpers; `replay_typesense_test.sh` is the supported public replay entrypoint there, while `prewarm_e5_small_model.sh` is a support helper behind it and CI staging.
...
- Helper-contract drift was fixed too: `test/scripts/replay_typesense_test.sh` now checks for the actual prewarmed model cache path (`ts_e5-small/model.onnx`) instead of a stale slash-separated path that caused unnecessary re-downloads.
...
- **Active now:** no queued modernization item currently remains in this plan. Finish monitoring the in-flight `release-binaries` run and open a successor item only if a lane fails or a new modernization gap is discovered.
...
- **Recently finished:** item **34** (`Replace patch-only forks with released upstream versions where possible`) — the stale `typesense/hnswlib` fork is gone. The repo now pins the same upstream `nmslib/hnswlib` parent commit directly and the canonical Dockerized `//:typesense-server` build is green on that change. The upstream `v0.8.0` release itself remains a documented no-go because it lacks the `searchKnnCloserFirst(..., ef, filter)` overload and `repair_zero_indegree()` APIs that `src/index.cpp` currently uses; `kakasi` and `clip_tokenizer_cpp` remain explicit exceptions in `bazel/PATCH_DEBT.md` rather than silent future work.
...
- **Later:** revisit the documented fork exceptions (`kakasi`, `clip_tokenizer_cpp`) only if a real upstream project/release path or a fresh upstream release appears. Monitor future sanitizer warning growth when GCC/libstdc++ or Abseil changes again, but keep any suppression file-scoped and dependency-scoped rather than broad. Mine `TODO.md` only when an item clearly aligns with the modernization queue above.
