...
| 19 | NuRaft replacement feasibility sprint | P1.7d | **in progress** | Investigation says NuRaft is the only serious in-process replacement candidate, but it is a real subsystem rewrite, not a dependency bump. Stories A and B are complete, the `ReplicationService` seam is landed, and `//:typesense-server-nuraft-prototype` now performs isolated startup preflight plus request-journal append/replay against the prototype segment log. Next step is to replace ad hoc CLI-only replay with a real prototype write/state-machine path and then tackle bounded tail-log recovery semantics. |
