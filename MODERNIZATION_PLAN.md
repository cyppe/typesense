...
| 19 | NuRaft replacement feasibility sprint | P1.7d | **in progress** | Investigation says NuRaft is the only serious in-process replacement candidate, but it is a real subsystem rewrite, not a dependency bump. Stories A and B are complete, the `ReplicationService` seam is landed, and `//:typesense-server-nuraft-prototype` now has isolated startup preflight, request-journal append/replay, replay-progress tracking, torn-tail recovery, structured applied-request records, startup recovery/apply automation, coarse route-intent classification, and a state-machine sink seam. Next step is to implement a non-file-backed prototype apply handler that begins exercising real Typesense-side write/apply behavior behind that seam. |
