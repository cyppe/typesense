...
| 23 | Compile warning audit / cleanup | P1 Hygiene | **active** | In progress on March 14, 2026. First-party `src/system_metrics.cpp` warning debt is fixed in source, and `.bazelrc` now narrows protobuf-only GCC suppressions to `external/protobuf+.*` (`-Wsign-compare`, `-Wmaybe-uninitialized`, plus `map.cc` deprecation noise) instead of carrying a broader Clang-only flag into GCC builds. Local proof so far: `scripts/bazel_in_docker.sh build //:typesense-server` and a cold-cache `TYPESENSE_BAZEL_CACHE_DIR=/home/cyppe/tmp/typesense-warning-audit/cache scripts/bazel_in_docker.sh build //:typesense-test` both finished with no visible compiler warnings beyond the known Bazel/OpenJDK startup banner. A follow-up home-cache ASAN replay (`TYPESENSE_BAZEL_CACHE_DIR=/home/cyppe/tmp/typesense-warning-audit/asan-cache scripts/bazel_in_docker.sh build --config=asan //:typesense-server`) also stayed warning-clean through startup, analysis, and the foreign-build phase before it was intentionally stopped to avoid leaving a long-running background build. Next step: complete the ASAN and TSAN replays, then only add more suppressions when the owner and scope are proven. |
| 24 | Release artifact debug-info policy | Known Issues | **pending** | Decide whether user-facing release binaries should keep embedded debug info, make it optional, or publish stripped binaries plus separate debug-symbol artifacts/packages. Current Linux release artifacts are much larger than Darwin because they ship as unstripped/self-contained binaries with embedded debug info; treat size vs debuggability as an explicit release policy instead of an accident. |

### Backlog map (active / later / archival)

Use this to decide what to pick next without scanning multiple files.

- **Active now:** item **23** (`Compile warning audit / cleanup`) — first-party `src/system_metrics.cpp` warnings are fixed in source, `.bazelrc` now scopes protobuf-only GCC suppressions narrowly, the fresh normal replays (`//:typesense-server` plus a cold-cache `//:typesense-test`) are clean apart from the known Bazel/OpenJDK startup banner, and the follow-up ASAN replay stayed clean through startup/analysis/foreign builds before being stopped intentionally. Next proof needed: completed ASAN and TSAN replays to determine what visible noise is still real code versus audited third-party/toolchain startup.
- **Recently finished:** item **22** (`Release workflow promotion / current-tip full-matrix replay`) — run `23088513187` succeeded on `linux-amd64`, `linux-arm64`, `darwin-arm64`, and `darwin-amd64`; the matching publish dry-run on version label `0.0.0-a2832b63` logged all `12` expected uploads after the RPM-glob fix. Item **20** (`Sanitizer lane stabilization + ORT extensions boundary audit`) — default ASAN is green again after removing `new_delete_type_mismatch=0` from `.bazelrc`, and the remaining ORT issue was resolved by rewriting fetched `OrtOpLoader` statics to process lifetime inside `bazel/onnxruntime.patch`. Item **21** (`NuRaft import/runtime parity + benchmark refactor sprint`) — hosted `benchmark-testing` is green on `27bb2bff` after the final replay-model fix. Item **18** (`Dependency refresh audit`) — all actionable deps at latest, patch debt at minimum. Item **19** (`NuRaft cutover`) — 120/120 API tests. Item **9** (`Protobuf 34`) — 34.0.bcr.1.
...
- **Next likely lane after item 23:** item **24** (`Release artifact debug-info policy`) — decide whether release binaries should keep embedded debug info, expose an optional stripped path, or publish split debug-symbol artifacts, then update the release workflow/docs to match that policy.
- **Later (planned but not started):** Finish the remaining compile warning audit buckets across normal, ASAN, and TSAN builds: audit `abseil-cpp` C++20/TSAN warnings, confirm whether the GCC 14 `libstdc++` / `std::regex` warnings still reproduce via `clip_tokenizer`, and keep startup-noise handling (`-Xverify:none`) tool-specific rather than broad. Env-dependent test suites (blocked on infrastructure). Abseil `20260107.1` upgrade (blocked by ORT ABI mismatch). Release workflow follow-up: keep local Linux replay as the first validation step, because stale smoke-test assertions like the old `Command line usage:` grep can break packaging even when the binary itself is healthy, and consider extracting Linux tarball/package assembly into a repo-owned wrapper so CI and local replay stop depending on workflow-only shell blocks.
...
41. **Keep image-embedder teardown ahead of text-embedder teardown anywhere model references are released.** `CollectionManager::dispose()`, `test/main.cpp`, and `process_embedding_field_delete()` should all drop image embedders before text embedders so the ORT Extensions-backed sessions leave the process before later session-only cleanup runs.
