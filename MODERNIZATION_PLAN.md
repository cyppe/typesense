...
1. [ ] Re-check the real DDEV lane on a fresh image built from the post-helper-rewrite branch head.
   Current state: the March 22, 2026 local `category_fanout` and `mixed_category_fitment` lanes are now green on current HEAD, including the harsher `300k` fanout / `200k` fitment / `thread_pool_size=4` mixed replay where the long `categories_se` import took `24.4s` but concurrent fitment imports still averaged `140.6ms` with `/health 0.3ms`, `/metrics.json 1.6ms`, and zero replay-side failures. The same date also added a generic late-reference fanout policy in the async-reference helper: moderate `1M` late vehicle fanout stays monolithic and improved from `6353.2ms` to `5579.2ms`, while larger `3M` late fanout switches to bounded `50k` helper chunks and still improved slightly (`19197.8ms -> 19002.2ms`) with zero slow requests. The earlier Laravel timeout evidence came from an older image before the async-reference helper rewrite, so the next meaningful gate is DDEV on a freshly built image from this newer runtime.
...
|---|------|---------|--------|-------|
| 25a | Build and publish a fresh DDEV test image from the post-snapshot-distance branch head | P1 Runtime/Perf | **active** | March 22, 2026 DDEV validation on the older image still timed out around `product_vehicle_fitments_se` once the job stream crossed the familiar `~9.8M` docs mark. New local evidence now isolates the remaining issue to aggressive NuRaft snapshotting on that older runtime: live gdb stacks during the stall showed `raft_server::snapshot_and_compact()` on the commit thread, DDEV startup logs still reported `snapshot_distance=10000`, and the update-heavy local replay now stays healthy by default on `snapshot_distance=100000` while the same replay times out again when forced back down to `10`. Next gate: build/publish the fresh branch head and rerun DDEV on that image before reopening deeper runtime work. |
| 25b | Re-check DDEV on the new image with the March 22 helper + snapshot telemetry enabled | P1 Runtime/Perf | **pending** | Use the fresh image from item 25a and keep the current Laravel/Typesense telemetry in place. If DDEV still stalls, capture the new `nuraft_snapshot_*` metrics plus `event=slow_request` helper fields before changing code again. |
