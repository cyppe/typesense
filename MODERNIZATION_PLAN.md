...
- **ICU dependency:** `nuraft_prototype_lib` requires `@icu` in BUILD deps because it transitively includes `string_utils.h` → `unicode/normalizer2.h`.
...
| 19 | NuRaft replacement cutover and hardening | P1.7d | **in progress** | The decision phase is over on this branch: `braft`/`brpc`/`glog` are removed, `//:typesense-server` is now the NuRaft-backed runtime. Single-node restart crash and multi-node non-CRUD replication are now fixed — `synonym_sets` passes all 6 phases (20/20 tests). Remaining: full `--no-secrets` sweep, remaining route families, async/import/streaming hardening. |
