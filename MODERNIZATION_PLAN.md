...
1. [ ] Re-check the real DDEV lane on a fresh image built from the post-helper-rewrite branch head.
   Current state: March 22, 2026 DDEV on the published image `2c06d882` cleared the old `~9.8M` snapshot stall, but it still exposed one remaining generic helper regression: a `categories_se` import rewrote `177,813` `products_se` docs, fetched about `2.14 GiB` of stored product JSON, stayed single-chunk, and produced a `33.99s` slow request. Current local HEAD now fixes that with a byte-aware helper planner: once the matched set exceeds `100k` docs, sample stored-doc size and only chunk if estimated fetched bytes exceed about `512 MiB`, with a `50k`-doc floor. The updated local replay set is green: `1M` late fitment fanout stays monolithic and improves to `5121.1ms`, `3M` fitment fanout plans `2` large chunks and improves to `18500.2ms`, and the new DDEV-parity `category_fanout 180k / 711 / 12KB` lane plans `4` chunks, completes in `18.9s`, and keeps `/health 0.6ms`, `/metrics.json 3.7ms`, and search `32.8ms` avg.
