...
| 20 | Sanitizer lane stabilization + ORT extensions boundary audit | Known Issues | **in progress** | Local-first stabilization work: normal `//:typesense-test` passes, full local ASAN is now green (`1449` tests), and the focused TSAN subset is green after skipping non-actionable Whisper voice-query inference under TSAN. Root cause for the prior suite-only `_rand(seed)` failure was a missing random-sort sentinel in `populate_sort_mapping()`, which left `field_values` undefined and could route scoring into the wrong branch under ASAN. Remaining work is a manual GitHub sanitizer rerun on the validated local change set and the longer-term ORT Extensions registration cleanup while keeping the promoted binary self-contained. |
