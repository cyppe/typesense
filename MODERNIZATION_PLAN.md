...
| 19 | NuRaft replacement feasibility sprint | P1.7d | **in progress** | Investigation says NuRaft is the only serious in-process replacement candidate, but it is a real subsystem rewrite, not a dependency bump. Stories A and B are complete, Story C's single-node boot/write/restart/replay checkbox is now done, and `//:typesense-server-nuraft-prototype` now has isolated startup preflight, request-journal append/replay, replay-progress tracking, torn-tail recovery, structured applied-request records, startup recovery/apply automation, coarse route-intent classification, and a RocksDB-backed materialized sink that now replays chunked imports into deterministic document state across restarts. Next step is static three-node append/commit, leader discovery, and follower catch-up while keeping the prototype isolated from `//:typesense-server`. |

### Backlog map (active / later / archival)

Use this to decide what to pick next without scanning multiple files.

- **Active now (execution lane):** item **18** (`Dependency refresh audit`) remains the primary modernization lane. Item **19** (`NuRaft replacement feasibility sprint`) is also in progress by explicit user request; Stories A/B are done and Story C now has the single-node contract checked off, so the next NuRaft work should stay bounded to the remaining Story C/D sprint items rather than turning into an unscoped rewrite.
