...
90. **Startup recovery readiness and steady-state read admission cannot share the same “lag exists” switch.** The March 31, 2026 full `//:typesense-test` lane caught a regression where the new recovery hardening made `is_materialization_ready()` flip false for any mirror-worker lag and `is_read_caught_up()` reused that signal, so follower searches during ordinary concurrent imports returned `503 Not Ready or Lagging`. Keep startup/snapshot recovery tracking explicit (`startup_materialization_pending`, snapshot-reload target, catch-up state) and let the steady-state search path use the latest locally applied state once the node is out of startup recovery.
91. **Snapshot install must rebuild the target node's `cluster_config.bin` from its own persisted identity/bootstrap metadata.** Deleting `cluster_config.bin` outright let small subprocess recovery tests pass but left a real recovered DDEV follower unable to forward writes or report a leader after snapshot restore; blindly preserving the source node's serialized cluster config breaks single-node snapshot installs. The safe rule is: keep the target node's identity/bootstrap files, drop the source `srv_state`, and regenerate `cluster_config.bin` for the target membership view. Coverage also needs an explicit recovered-follower write path, not just leader-originated post-recovery writes.
92. **Persist the snapshot's real Raft term, not only its log index.** The remaining DDEV rejoin failure after the image-rebuild work came from restarted leaders reloading `last_snapshot_ptr_` with `last_log_term=0`, so they served a logically valid snapshot whose boundary term no longer matched the real leader log. Empty followers could install the snapshot and reload the main `db/`, but then rejected every subsequent append-entry at the snapshot boundary, leaving leader peer progress stuck at `last_log_idx=0`, follower `raft_leader_id=-1`, and follower-originated writes failing with `Leader is server -1`. Persist `last_log_term` in `last_snapshot.bin`, reload it into `last_snapshot_ptr_`, and cover the DDEV-shaped case with a test that restarts the cluster before wiping the follower.
93. **Graceful-stop resilience tests are not enough to cover real quorum-loss behavior.** The April 1, 2026 DDEV crash report happened after hard-killing a follower, then the leader, while traffic kept hitting the lone survivor. Our existing `api_tests/tests/nuraft_cluster_resilience.test.ts` only used `stopMultiNodeServer()` / `restartMultiNodeServer()`, so it never exercised true `SIGKILL` quorum loss or the restart of dead peers under continued write pressure. The new hard-kill coverage must remain in place even though the local latest-build replay did not reproduce a segfault under ASAN: it still validated the important invariant that the lone survivor stayed alive, the cluster re-elected a leader after peers returned, and follower-originated writes worked again.
