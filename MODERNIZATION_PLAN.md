...
- **Prototype materialized apply sink landed:** the new optional `NuRaftKvStateMachineSink` persists applied-request history plus a small RocksDB-backed materialized view under `state/nuraft-prototype/materialized_state`, mapping collection create/drop, document write/delete, and import routes into deterministic collection/document/import keys. The prototype CLI now accepts `--state-machine-sink=kv` and `--dump-materialized-state` so single-node append/restart/apply can exercise a closer Typesense-style state mutation path without touching `//:typesense-server`, covered by expanded `//:nuraft-state-machine-sink-test` and `//:nuraft-replication-controller-test` coverage.
...
| 19 | NuRaft replacement feasibility sprint | P1.7d | **in progress** | Investigation says NuRaft is the only serious in-process replacement candidate, but it is a real subsystem rewrite, not a dependency bump. Stories A and B are complete, and `//:typesense-server-nuraft-prototype` now has isolated startup preflight, request-journal append/replay, replay-progress tracking, torn-tail recovery, structured applied-request records, startup recovery/apply automation, coarse route-intent classification, a sink seam, and an optional RocksDB-backed materialized apply sink. Next step is to widen that materialized path toward real batched-indexer / Typesense-side replay semantics while keeping it isolated from `//:typesense-server`. |

### Backlog map (active / later / archival)

Use this to decide what to pick next without scanning multiple files.

- **Active now (execution lane):** item **18** (`Dependency refresh audit`) remains the primary modernization lane. Item **19** (`NuRaft replacement feasibility sprint`) is also in progress by explicit user request; Stories A/B are now done, and any further work should stay bounded to the sprint plan rather than turning into an unscoped rewrite.
