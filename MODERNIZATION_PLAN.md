...
- [x] Backport the smallest safe user-visible fixes first: zero-match phrase vector behavior. *(`#2793` diversity seq-id ordering, `#2772` `/health` meta-thread-pool routing, `#2809` model-download timeout bump, `9b0d729b` model-config fetch timeout bump, `#2817` zero-match phrase/vector fallback, `c2272a95` nested preset auth hardening, `5bef9362` filtering/not-equals cleanup, `ae64826a` delete-by-query batching, `1b4fa888` highlight-race hardening, and `3ad9d3e0` highlight posting-list de-cloning are now landed locally. The behavior fixes carry focused tests; the timeout-only backports are documented as non-trivial to exercise without an injected transport seam.)*
- [ ] Re-evaluate the remaining search-locking item (`5458d3f0`) against current NuRaft-era code before touching it; do not assume it is safe just because it shipped on the classic release line.
