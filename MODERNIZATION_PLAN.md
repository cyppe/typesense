...
- **Coarse runtime-parity timing now also favors the bounded NuRaft lane:** timing the same real `api_tests` files through `scripts/run_api_tests.sh --skip-install` showed `tests/collections.test.ts` completing in about `2.55s` on `//:typesense-server-nuraft-runtime` versus `59.30s` on `//:typesense-server`, and `tests/documents.test.ts` in about `5.15s` versus `57.80s`. This is useful apples-to-apples runtime-path evidence on implemented surfaces, but it is still a coarse end-to-end wrapper timing, not a replacement for the canonical benchmark lane.
