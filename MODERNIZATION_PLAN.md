...
| 38 | Indexing hot-path string-copy audit | P2 Perf | **active** | Run 29 still stands as the only valid measured compare. Two Mar 18 repeat attempts on local `1da111f7` with the default benchmark workdir still hit Dockerized k6/Influx `mkdir /var/lib/influxdb/data` errors, so the benchmark harness currently cannot supply the clean repeat needed to close the item. |
...
- **Active now:** item **38** remains in flight. Run 29 still shows a plausible import win from the move-only `index_record` handoff change, but two Mar 18 repeat attempts on local `1da111f7` still hit the Dockerized k6/Influx `mkdir /var/lib/influxdb/data` failure even on the default benchmark workdir, so the benchmark harness must be repaired or bypassed before the compare can be closed honestly. Item **39** is the next product-facing modernization task after that harness block is resolved or explicitly handled.
