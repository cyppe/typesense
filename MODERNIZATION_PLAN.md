...
- **Steady write-path signals exist now too:** the same canonical `raft-recovery` command now includes a simple single-node write comparison. In the latest `docs=200` repeats, the NuRaft prototype append/apply path handled the batch in about `0.87ms` append + `16.09ms` apply with roughly `20ms` total process CPU and about `27MB` peak RSS, while the live single-node `braft`/HTTP server path took about `177.81ms`, roughly `60ms` process CPU, and roughly `520MB` peak process RSS in those runs. Treat these as directional write-path signals only, not final replacement-grade throughput numbers, because they still compare an isolated prototype binary against the real server runtime.
