...
| 33 | ~~GPU deps artifact automation boundary~~ | P2 Release | **done** | Completed March 16, 2026. The real blocker was the branch's one-Protobuf CUDA server path, not DEB/RPM conversion: `@onnx_runtime//:onnxruntime_static_one_protobuf --define=use_cuda=on` initially failed under external Protobuf 34, then was unblocked with a repo-owned protobuf compatibility patch. Story B added the canonical `scripts/release_linux_gpu_deps.sh` tarball/package producer, wired a sibling `release-gpu-deps.yml` workflow, updated Linux `release-binaries` to build the regular `typesense-server` artifact with `--define=use_cuda=on`, and kept docs honest that the GPU surface here is ONNX Runtime embeddings/personalization only (Whisper remains CPU-only). |
| 34 | Replace patch-only forks with released upstream versions where possible | P1.7 Patch Debt | queued | Use `bazel/PATCH_DEBT.md` as the owner, prefer upstream releases over carried patches, and verify any patch-droppability claim with a full Dockerized Bazel build. |
| 35 | Script / doc entrypoint architecture cleanup | P2 DX | queued | User-requested follow-up. Audit wrapper naming, folder ownership, stale helper scripts, and duplicated doc references so each task class has one obvious canonical entrypoint and the surrounding docs point to it cleanly. |
...
- Ranked remaining host-dependent flows:
  ...
  3. `test/scripts/prewarm_e5_small_model.sh` plus the runbook/workflow `curl` fetches: **low value, low reproducibility gain**. They still assume host `curl`, but that dependency is ubiquitous on CI/Linux hosts and wrapping simple downloads in Docker would mostly duplicate an acceptable flow.
...
- Go. The repo already had the right downstream contract for this variant: `debian-pkg/generate_deb_rpm.sh` and `publish_release.sh` were already suffix-aware, jemalloc already had an arm64 `lg_page16` config gate, and the missing surface was the canonical release wrapper/workflow, not a second release family.
...
- Linux `typesense-server` release lanes now build with `--define=use_cuda=on` via `scripts/release_linux_artifacts.sh --with-cuda`, while the optional GPU sidecars remain a sibling artifact class in `.github/workflows/release-gpu-deps.yml`.
- `publish_release.sh` now uploads both `typesense-server-*` and `typesense-gpu-deps-*`, and the public repo docs (`README.md`, `TESTING_RUNBOOK.md`, `AGENTS.md`) now describe the real GPU scope and canonical replay entrypoints.

**Release boundary decision**

1. Keep `typesense-gpu-deps` as a **sibling workflow / artifact class**. This is the cleanest maintenance boundary because the optional ORT provider sidecars are a distinct release contract from the main server/debug artifacts.
2. Keep Linux `typesense-server` inside `release-binaries`. The upstream-style install contract requires the regular Linux server artifact itself to be CUDA-aware, and this only adds one build flag plus the CUDA build image to the existing canonical wrapper.
3. Do **not** add a reusable workflow yet. There is still only one stable GPU-deps caller, so extra workflow abstraction would add maintenance without improving reproducibility.
...
- [x] The plan records the exact GPU artifact contents and the clean release boundary. *(Decision: keep the optional sidecars in the sibling `release-gpu-deps` workflow while Linux `release-binaries` builds the regular server artifact with `--define=use_cuda=on`.)*
