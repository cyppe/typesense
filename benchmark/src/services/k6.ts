...
export class K6Benchmarks {
  ...
  private extractIndexBenchmarkExecutionResult(
    ...
    const summaryMatch = /K6_INDEX_SUMMARY_JSON:(\{.*?\})/.exec(output);
    if (summaryMatch) {
      try {
        const parsed = JSON.parse(summaryMatch[1]) as { checksPassed?: number; checksTotal?: number; importDurationMs?: number };
        if ((parsed.checksTotal ?? 0) > 0 && parsed.checksPassed !== parsed.checksTotal) {
          return errAsync({
            message: `Index benchmark summary reported failed checks (${parsed.checksPassed ?? 0}/${parsed.checksTotal ?? 0})`,
          });
        }

        if (typeof parsed.importDurationMs === "number" && Number.isFinite(parsed.importDurationMs) && parsed.importDurationMs >= 0) {
          return okAsync({
            importDurationMs: parsed.importDurationMs,
          });
        }
      } catch {
    ...
    const legacySummaryMatch = /Index benchmark summary .*?\bimport_duration_ms=(\d+)/.exec(output);
    ...
    return okAsync({
      importDurationMs: Number.parseInt(legacySummaryMatch[1], 10),
    });
  }
