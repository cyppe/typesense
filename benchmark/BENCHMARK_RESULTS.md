...
- Hosted CI also uses a larger explicit k6 indexing `maxDuration` (`30m`) than the default executor ceiling, because the smaller CI chunking makes the single import iteration long enough to hit k6's default `10m` cap on GitHub's hosted runners.
