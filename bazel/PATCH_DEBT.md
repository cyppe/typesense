...
1. Prioritize shrinking `bazel/whisper.patch` and `bazel/icu/icu.patch` (largest maintenance risk). `bazel/onnxruntime.patch` was re-audited on ORT `1.24.3` and is still non-droppable for now.
