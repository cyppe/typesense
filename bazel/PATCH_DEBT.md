...
1. Prioritize shrinking `bazel/whisper.patch` and any remaining non-product-specific build patches. `bazel/icu/icu.patch` has been reduced to the AR fix only, and `bazel/onnxruntime.patch` was re-audited on ORT `1.24.3` and is still non-droppable for now.
2. Re-audit this file after any dependency version bump in `MODULE.bazel`.
